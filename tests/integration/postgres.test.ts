...
test("isolated Postgres migration, concurrent create, rollback and replacement", { skip: !connection, timeout: 90_000 }, async () => {
  ...
  assert.equal(await db.getIssue(date), null, "Test date must be empty; never replace pre-existing data");
  const issue = { ...seedIssue, date, title: "Isolated concurrent publication test" };
  try {
    const results = await Promise.all(Array.from({ length: 12 }, () => db.createIssue(issue)));
    assert.equal(results.filter((result) => result.created).length, 1);
    assert.equal(new Set(results.map((result) => result.issueId)).size, 1);
    const stored = await db.getIssue(date);
    ...
    await assert.rejects(() => db.replaceIssue({ ...invalid, title: "Must roll back" }));
    assert.equal((await db.getIssue(date))?.title, issue.title);
    await db.replaceIssue({ ...issue, title: "Replaced", sections: [issue.sections[0]] });
    assert.equal((await db.getIssue(date))?.sections.length, 1);
    assert.equal((await db.getIssue(date))?.title, "Replaced");
  } finally {
    await sql`DELETE FROM issues WHERE issue_date = ${date}`;
  }
});
