...
1. **ChatGPT OAuth:** configure an established OAuth provider, resource/audience, JWKS URL, and owner subject using `MCP.md`; then switch the environment from `pilot` to `oauth` and redeploy. The bearer pilot cannot establish the ChatGPT connection by itself.
